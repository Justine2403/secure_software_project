...
urls = [ '/employers', '/.gitignore', '/~admin', '/administrator.log', '/downloads.html','/employers.html']
urls_304 = ['/award','/research', '/research.log']

async def app(scope, receive, send):
    assert scope['type'] == 'http'
    await send({
        'type': 'http.response.start',
        'status': 200 if scope['path'] in urls else (304 if scope['path'] in urls_304 else 404),
        'headers': [
